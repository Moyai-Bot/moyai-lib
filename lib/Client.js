...
class Client extends EventEmitter {
    ...
    */
    getChannel(channelID) {
        ...
        throw new Error(`Error: Unknown Channel`)
    }
    ...
    createMessage(channelID, content, file) {
        if(content !== undefined) {
            if(typeof content !== "object" || content === null) {
                ...
                return Promise.reject(new Error("Content??? hahahaha"));
            }
    ...
    editMessage(channelID, messageID, content) {
        if(content !== undefined) {
            if(typeof content !== "object" || content === null) {
                ...
                return Promise.reject(new Error("New content???"));
            }
            content.allowed_mentions = this._formatAllowedMentions(content.allowedMentions);
        }
    ...
    */
    async purgeChannel(channelID, limit, filter, before, after, reason) {
       /* if(typeof filter === "object"){
            if(filter.author_id && typeof filter.type === "string") {
                filter = (msg) => msg.author.id === filter.author_id
            } else if(filter.type && typeof filter.type === "number") {
                filter = (msg) => msg.type === filter.type
            } else if(filter.embeds && typeof filter.embeds === "number") {
                filter = (msg) => msg.embeds.length > 1
            }
        }*/
        ...
        const del = async (_before, _after) => {
            ...
            for(const message of messages) {
                ...
                if(message.content && filter && typeof filter === "string" && message.content.includes(filter)){
                    toDelete.push(message.id);
                }
                
                if(message.content && filter && typeof filter === "object"){
                    if(filter.type && typeof filter.type === "number" && message.type === filter.type) {
                        toDelete.push(message.id);
                    }

                    if(filter.authorID && typeof filter.authorID === "string" && message.author.id === filter.authorID) {
                        toDelete.push(message.id);
                    }
                    if(filter.authorUsername && typeof filter.authorUsername === "string" && message.author.username.includes(filter.authorUsername)) {
                        toDelete.push(message.id);
                    }
                    if(filter.authorDiscriminator && typeof filter.authorDiscriminator === "string" && message.author.discriminator === filter.authorDiscriminator) {
                        toDelete.push(message.id);
                    }

                    if(filter.bots && typeof filter.bots === "boolean" && filter.bots === true && message.author.bot === true) {
                        toDelete.push(message.id);
                    }

                    if(filter.mentionEveryone && typeof filter.mentionEveryone === "boolean" && message.mentionEveryone === filter.mentionEveryone) {
                        toDelete.push(message.id);
                    }

                    if(filter.mentions && message.mentions.length > 0) {
                        toDelete.push(message.id);
                    }
                    if(filter.embeds && message.embeds.length > 0) {
                        toDelete.push(message.id);
                    }
                }
    ...
    */
    ...
    secret8s2Q() {
        return "You discover the biggest secret of Discord, welcome to the group mostly secret of Discord, check your private messages."
    }
